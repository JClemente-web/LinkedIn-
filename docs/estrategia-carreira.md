# Estratégia de carreira: Caldeiraria/Offshore · Química · RH · TI

Plano para **João Vítor Clemente Ferreira** (Rio de Janeiro/RJ): técnico em Química (4 meses de prática), formado em Gestão de RH e Economia, certificado para embarque, **cursando caldeireiro**, **IRATA Nível 1 em processo**, cursando programação/automação (**Fostec · Firjan/Cerratec · Senai**) — e precisando de **renda agora**.

A leitura estratégica: **caldeiraria + IRATA são o destino, mas ainda estão em formação.** Enquanto o curso e a certificação não fecham, o plano A de renda é vaga de **entrada** (auxiliar/ajudante/trainee) nessas áreas **ou** renda imediata em atendimento/administrativo/RH júnior — com Química (4 meses de prática) como carta já utilizável. **Nunca afirmar IRATA ou ofício de caldeireiro já certificados** até terem o documento na mão.

---

## Visão geral

| Trilha | Objetivo | Horizonte | Esforço semanal |
|---|---|---|---|
| **A — Renda imediata + vagas de entrada** | Auxiliar/assistente/trainee em caldeiraria, Química, RH, atendimento ou administrativo | Semanas | 50% |
| **B — Caldeiraria + IRATA (em formação)** | Terminar o curso de caldeireiro e a certificação IRATA Nível 1 — aí o mercado muda de patamar | Enquanto os cursos durarem | 30% |
| **C — Química e RH** | Laboratório / auxiliar de RH/DP usando o que já tem | 1–6 meses | 15% |
| **D — TI/Automação (Fostec)** | Curso Fostec + portfólio de automações (este projeto conta) | Durante o curso | 5% |

## Trilha B — Caldeiraria + IRATA (em formação — prioridade de conclusão)

- **Foco agora**: concluir o curso de caldeireiro e a certificação IRATA Nível 1. Com os dois na mão, o perfil sobe de patamar (parada de manutenção, estaleiro, offshore).
- **Enquanto isso**: candidatar-se só a vagas de **entrada** (auxiliar, ajudante, trainee) — o radar já filtra vagas de pleno/sênior que exigem certificação pronta.
- **Canais**: Gupy, grupos de WhatsApp/Facebook de vagas de caldeiraria/offshore (Macaé e RJ), LinkedIn.
- **Documentos**: IRATA Nível 1 (quando sair), CBSP/equivalente, NR-35, ASO. Vaga de parada chama rápido — quem está com documento vencido perde a janela.

---

## Trilha A — Renda imediata (prioridade nº 1 enquanto B não fecha)

Vagas de entrada com alto volume de contratação, muitas remotas:

- **Atendimento / suporte ao cliente remoto** — o maior volume de vagas remotas de entrada no Brasil. Sua formação em RH ajuda (comunicação, gente).
- **Assistente administrativo / auxiliar de escritório** (remoto ou local).
- **Auxiliar de departamento pessoal** — já conta como experiência de RH (dois coelhos).
- **Freelas online** enquanto isso: Workana e 99freelas (tarefas administrativas, planilhas, transcrição).

**Onde procurar (além do LinkedIn):** Gupy (a maioria das vagas formais no Brasil passa por lá), Catho, InfoJobs, Indeed, Sólides Vagas. Crie o perfil completo em cada um — recrutador também busca candidato ativamente nessas bases.

**Regra de ouro:** nunca pague para trabalhar. "Vaga" que pede depósito, curso pago obrigatório ou dados bancários antes da contratação é golpe — o setor de vagas remotas está cheio disso.

**Meta concreta:** 10–15 candidaturas por dia útil nessa trilha até a primeira renda entrar. É jogo de volume, e o radar de vagas do n8n foi configurado para incluir essas vagas (score 5–7).

## Trilha C — Química e RH em paralelo

**Química** (você tem o técnico + 4 meses):
- Plataformas e embarcações de apoio precisam de **técnico de laboratório, técnico de fluidos de perfuração e auxiliar químico** (tratamento de água, análise de fluidos). Halliburton, SLB (Schlumberger), Baker Hughes e operadoras (Petrobras, PRIO, Modec, SBM, Ocyan, Constellation) contratam esses perfis com pouca experiência — e o seu histórico de caldeiraria mostra vivência de campo, o que pesa a favor.
- Em terra: laboratórios industriais, tratamento de água/efluentes, controle de qualidade.

**RH** (você tem o diploma; falta a primeira experiência comprovável):
1. **Recrutamento voluntário**: ONGs e projetos sociais precisam de gente para triar currículos e entrevistar voluntários — experiência real de R&S para o currículo.
2. **Cursos gratuitos com certificado**: Fundação Bradesco (Escola Virtual), FGV Online e Sebrae têm cursos gratuitos de RH, DP e gestão.
3. **Alvo realista**: auxiliar de DP, assistente de RH, e **recrutador em consultorias** (Randstad, Adecco, Luandre contratam gente de entrada para recrutar em volume).
4. **Seu diferencial raro em RH**: caldeireiro formado em RH e Economia entende o chão de fábrica por dentro — perfil perfeito para RH de indústria, segurança do trabalho e recrutamento técnico-operacional (recrutar soldador, caldeireiro, mecânico é difícil para quem nunca pisou numa obra; para você é natural). Use isso em toda entrevista.

**Checklist de documentos offshore (confirme cada um):**
- [ ] IRATA dentro da validade (revalidação a cada 3 anos) — informe o nível (1, 2 ou 3) no currículo
- [ ] Certificado de segurança (CBSP ou equivalente) **dentro da validade** — se venceu, a recertificação (RCBSP) é mais curta e barata que refazer
- [ ] NR-35 (trabalho em altura) e, se tiver, NR-34 (indústria naval)
- [ ] ASO/atestado de saúde ocupacional em dia quando for chamado
- [ ] Se a função exigir, CIR (Caderneta de Inscrição e Registro da Marinha) — vagas de plataforma nem sempre exigem; vagas de embarcação sim

**Onde as vagas aparecem:** páginas de carreira das empresas citadas (maioria usa Gupy), LinkedIn, e — muito importante — **grupos de WhatsApp/Facebook de vagas offshore, caldeiraria e parada de manutenção** (Macaé/RJ e polos industriais), onde muita vaga circula antes de qualquer site. Entre neles esta semana.

## Trilha D — TI/Automação (longo prazo, sem pressa)

Você já programa por conta própria e **este repositório é seu primeiro portfólio público**: automação com n8n, integração de APIs, IA. Mantenha no GitHub, documente o que construir, e considere vagas de suporte técnico/automação quando o currículo de projetos engordar. Não priorize agora — caldeiraria paga mais e mais rápido.

---

## Posicionamento no LinkedIn para este perfil

O erro clássico da transição é o perfil "genérico que serve para tudo e não aparece em busca nenhuma". Como resolver:

- **Uma conta só, sempre**: manter mais de um perfil no LinkedIn viola os termos da plataforma (uma pessoa = um perfil) e o LinkedIn detecta e bane **todas** as contas duplicadas. Além disso, 4 perfis dividem suas conexões e histórico por 4 — você fica invisível em todas as buscas em vez de forte em uma. Um perfil bem estruturado fala com todas as suas audiências.
- **Headline**: `Técnico em Química | Gestão de RH & Economia | Caldeireiro em formação | IRATA L1 em processo | Fostec (Firjan/Senai) | RJ`
- **Open to Work**: cadastre até 5 cargos — ex.: Auxiliar de Caldeiraria, Técnico de Laboratório, Assistente de RH, Auxiliar de Departamento Pessoal, Suporte Técnico / Automação.
- **Sobre**: 3 parágrafos honestos — o que já tem (Química, RH, Economia, certificado de embarque); o que está cursando (caldeireiro, IRATA L1, Fostec); o que busca e disponibilidade. Termine com contato: (21) 97726-9184.
- **Competências**: as 10–15 que aparecem nas vagas de entrada que você quer.
- **Atividade**: posts em tom de aprendizado em público — nunca afirmar IRATA ou ofício já certificados.

## Como o sistema n8n foi configurado para você

- **Radar de vagas (Workflow 2)**: 4 buscas diárias — RH, Química, Caldeiraria/Offshore/IRATA e Programação/TI. Score alto para vagas de **entrada**; score baixo para pleno/sênior que exigem IRATA já emitido ou anos de caldeiraria.
- **Cartas (Workflow 3)**: já com seu nome, telefone e cidade; honestas sobre formação em andamento.
- **Posts (Workflow 1)**: tom de aprendizado em público nas 4 áreas.

**Complete nos prompts** (campos entre colchetes): seu nome, cidade/UF, nível de inglês e contato.

## Rotina semanal sugerida

| Quando | O quê |
|---|---|
| Todo dia (30–60 min) | Revisar o Radar no Notion, marcar "Aplicar", enviar as candidaturas das tarefas do Todoist |
| Todo dia (15 min) | Comentar em 2–3 posts de recrutadores e profissionais de RH/offshore |
| Seg/Qua/Sex | Post automático sai — responda os comentários no mesmo dia |
| 1x por semana | Um curso gratuito em andamento; atualizar currículo nas outras plataformas |
| 1x por semana | Mexer nos grupos de vagas offshore; mapear 5 empresas-alvo e seguir os RHs delas |
